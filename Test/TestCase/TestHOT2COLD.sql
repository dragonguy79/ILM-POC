...
-----------------------------------------------------------------------------------------------------------------
  ...
  ILM_TEST.TEST_SQL_COUNT(3, 'SELECT COUNT(*) FROM USER_LOB_PARTITIONS WHERE  TABLE_NAME=''TPAYMENTINTERCHANGECOLD'' AND PARTITION_NAME=''P2015_10'' AND tablespace_name=''ILM_COLD_TBS''');
  ...
  ILM_TEST.TEST_SQL_COUNT(48, 'SELECT COUNT(*) from USER_LOB_SUBPARTITIONS subpart, USER_LOB_PARTITIONS part where subpart.TABLE_NAME=''TPAYMENTINTERCHANGECOLD'' AND subpart.LOB_PARTITION_NAME=part.LOB_PARTITION_NAME and part.partition_name=''P2015_10'' AND subpart.TABLESPACE_NAME=''ILM_COLD_TBS'' ');
  ...
  ILM_TEST.TEST_SQL_COUNT(2, 'SELECT COUNT(*) FROM USER_LOB_PARTITIONS WHERE  TABLE_NAME=''TPAYMENTTRANSACTIONCOLD'' AND PARTITION_NAME=''P2015_10'' AND tablespace_name=''ILM_COLD_TBS''');
  ...
  -- test that subpartitioned lob are in COLD tablespace
    -- assert 48 subpartitioned lob (2 columns of LOB x 16 subpartition) in tbs COLD
  ILM_TEST.TEST_SQL_COUNT(32, 'SELECT COUNT(*) from USER_LOB_SUBPARTITIONS subpart, USER_LOB_PARTITIONS part where subpart.TABLE_NAME=''TPAYMENTTRANSACTIONCOLD'' AND subpart.LOB_PARTITION_NAME=part.LOB_PARTITION_NAME and part.partition_name=''P2015_10'' AND subpart.TABLESPACE_NAME=''ILM_COLD_TBS'' ');
  ILM_TEST.TEST_SQL_COUNT(32, 'SELECT COUNT(*) from USER_LOB_SUBPARTITIONS subpart, USER_LOB_PARTITIONS part where subpart.TABLE_NAME=''TPAYMENTTRANSACTIONCOLD'' AND subpart.LOB_PARTITION_NAME=part.LOB_PARTITION_NAME and part.partition_name=''P2015_11'' AND subpart.TABLESPACE_NAME=''ILM_COLD_TBS'' ');

  -- test lob indexes
    -- assert that all lob index (2 column x 3 partition(2 moved in + 1 existing P9999_12) x 16 sub partition) are in ILM_HOT_TBS tablespace for TPAYMENTINTERCHANGECOLD table
...
-----------------------------------------------------------------------------------------------------------------
  ...
  -- ******************** TPAYMENTTRANSACTION ********************-- 
   -- test that all global index are usuable
    -- assert no USUSABLE status, and tablespace of global index is not moved
  ILM_TEST.TEST_SQL_COUNT(0, 'SELECT COUNT(*) from USER_INDEXES ui where STATUS=''UNUSABLE'' AND TABLE_NAME in (''TPAYMENTTRANSACTION'', ''TPAYMENTINTERCHANGECOLD'')');
  
  -- test that no partitioned index moved in COLD tablespace
   -- because TPAYMENTTRANSACTION does not have any local index
  ILM_TEST.TEST_SQL_COUNT(0, 'SELECT COUNT(*) from USER_IND_PARTITIONS pi inner join USER_INDEXES ti on pi.index_name=ti.index_name where TABLE_NAME=''TPAYMENTTRANSACTION'' and pi.partition_name=''P2015_11''');
  
  -- test that no subpartitioned index moved in COLD tablespace
    -- because TPAYMENTTRANSACTION does not have any local index
