create or replace PACKAGE BODY ILM_CORE AS
  ...
  BEGIN
    FOR spRow in (select SUBPARTITION_NAME from USER_TAB_SUBPARTITIONS WHERE TABLE_NAME=I_TABLE_NAME AND PARTITION_NAME=I_PARTITION_NAME AND TABLESPACE_NAME=I_FROM_TBS )
    LOOP
