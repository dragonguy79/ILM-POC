...
BEGIN  
  FOR spRow in (select SUBPARTITION_NAME from USER_TAB_SUBPARTITIONS WHERE PARTITION_NAME='P2015_11')
    LOOP
      EXECUTE IMMEDIATE 'ALTER TABLE PAYMENTINTERCHANGE MOVE SUBPARTITION ' || spRow.SUBPARTITION_NAME  || ' TABLESPACE ILM_COLD_TBS ONLINE PARALLEL 4';
    END LOOP;
END;
/
  
-- split high bound partition in target table
ALTER TABLE paymentinterchangecold SPLIT PARTITION pc9999_12 AT (to_date('2015-12-01', 'yyyy-MM-dd')) INTO (PARTITION pc2015_11, PARTITION pc9999_12) PARALLEL 4;
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE IMMEDIATE 'ALTER TABLE PAYMENTINTERCHANGETEMPWARM MOVE PARTITION ' || pRow.PARTITION_NAME||' TABLESPACE ILM_HOT_TBS ONLINE PARALLEL 4';
  END LOOP;
END;
/
